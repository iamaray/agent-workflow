...
usage() {
  ...
  printf 'Creates a Claude feature workspace under .claude/features/<slug>.\n' >&2
}
...
printf 'Created %s\n' "$target"
printf 'Next: start claude at %s and invoke %s %s <feature brief>\n' "$script_dir" "$invocation" "$slug"
