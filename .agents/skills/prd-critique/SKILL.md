---
name: prd-critique
description: Audit and revise an in-review PRD against the current codebase with exactly four parallel read-only critics chosen from the PRD's likely failure surfaces. Use only when explicitly invoked as $prd-critique with a path to PRD.md; approve only when evidence, traceability, and product decisions are implementation-ready.
---
...
1. Resolve the PRD path and repository root. Require the PRD to be inside `.codex/features/` and its sibling `TDD.md` to exist.
2. Run `python3 scripts/validate_artifact.py <PRD-path> --phase review`.
3. Read the entire PRD, applicable `AGENTS.md`, repository structure, and current Git status.
