---
name: implement
description: Plan privately, implement, verify, document, and commit an approved TDD in the current repository. Use only when explicitly invoked as $implement with a path to TDD.md; may use zero to two bounded read-only subagents, must preserve unrelated work, and never pushes or deploys.
---
...
1. Resolve the TDD and sibling PRD under `.codex/features/`.
...
4. Read applicable `AGENTS.md`, the full documents, current Git status, and every cited path.
