---
name: prd-critique
description: Audit and revise an in-review PRD against the current codebase with exactly four parallel read-only critics chosen from the PRD's likely failure surfaces. Use when the user invokes /prd-critique with a path to PRD.md; approve only when evidence, traceability, and product decisions are implementation-ready.
...
1. Resolve the PRD path and repository root. Require the PRD to be inside `.claude/features/` and its sibling `TDD.md` to exist.
